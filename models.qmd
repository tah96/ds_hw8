...
recipe_1 <- recipe(rented_bike_count ~ ., data = bike_train) |>
  ...
  step_mutate(date_dow = factor(date_dow,levels=unique(date_dow),labels=if_else(unique(date_dow) %in% c('Mon','Tue','Wed','Thu'),"Weekday","Weekend"))) |>
...
recipe_3 <- recipe_2 #|>
  #step_poly(all_numeric(), degree = 2,keep_original_cols = FALSE)
...
Looking at our CV error (2 for each), we see that _____ is our best model!
...
```
Since _____ is our best model, we want to keep this, evaluate against our entire training set and test against our test set.

Our RMSE evaluated against our test data is _______. We can also see our coefficients from the fitted model in the second output module.


```{r}
test_metrics <- bike_wfl_2 |>
  last_fit(bike_split) |>
  collect_metrics()

final_model <- bike_wfl_2 |>
  fit(bike_train) |>
  extract_fit_parsnip() |>
  tidy()
